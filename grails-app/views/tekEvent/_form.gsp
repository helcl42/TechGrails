...
<div class="fieldcontain ${hasErrors(bean: tekEventInstance, field: 'description', 'error')} ">
	...
	<g:textArea name="description" cols="70" rows="5" maxlength="5000" value="${tekEventInstance?.description}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: tekEventInstance, field: 'organizer', 'error')} required">
	...
	<g:select id="organizer" name="organizer.id" from="${grails.TekUser.list()}" optionKey="id" required="" value="${tekEventInstance?.organizer?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: tekEventInstance, field: 'volunteers', 'error')} ">
	...
	<g:select name="volunteers" from="${grails.TekUser.list()}" multiple="multiple" optionKey="id" size="5" value="${tekEventInstance?.volunteers*.id}" class="many-to-many"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: tekEventInstance, field: 'sponsorships', 'error')} ">
    ...
    <ul id="sponsorships" class="one-to-many">
        ...
        <li class="add">
            <g:link controller="sponsorship" action="create" params="['tekEvent.id': tekEventInstance?.id]">
...
<div class="fieldcontain ${hasErrors(bean: tekEventInstance, field: 'tasks', 'error')} ">
    ...
    <ul id="tasks" class="one-to-many">
        ...
        <li class="add">
            <g:link controller="task" action="create" params="['tekEvent.id': tekEventInstance?.id]">
...
<div class="fieldcontain ${hasErrors(bean: tekEventInstance, field: 'messages', 'error')} ">
    ...
    <ul id="messages" class="one-to-many">
        <g:each in="${tekEventInstance?.messages?}" var="m">
            <li>
                <g:link controller="message" action="show" id="${m.id}">
                    ${m?.encodeAsHTML()}
                </g:link>
            </li>
        </g:each>
        ...
        <li class="add">
            <g:link controller="message" action="create" params="['event.id': tekEventInstance?.id]">
                ${message(code: 'default.add.label', args: [message(code: 'message.label', default: 'Message')])}
            </g:link>
        </li>
    </ul>
</div>
